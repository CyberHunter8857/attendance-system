    ...
    <header className="sticky top-0 z-40 border-b border-border bg-card shadow-sm">
      <div className="flex h-16 items-center gap-4 px-4 md:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="hidden text-lg font-semibold text-foreground md:block">
            Attendance System
          </span>
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="rounded-full overflow-hidden border border-border" aria-label="User menu">
                {user?.photo ? (
                  <img src={user.photo.startsWith('http') ? user.photo : `${user.photo}`} alt="Profile" className="w-full h-full object-cover" />
                ) : (
